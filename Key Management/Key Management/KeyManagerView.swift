...
public struct KeyManagerView: View {
	...
	@State
	private var doShowFileExporter = false
	
	@State
	private var selectedKeyPairs = Set<KeyPair>()
	
	@State
	private var selectedKeyPairForExport: KeyPair?
	...
	public var body: some View {
		List(self.keyPairs, selection: self.$selectedKeyPairs) { (keyPair) in
			...
			.fileExporter(
				isPresented: self.$doShowFileExporter,
				document: self.selectedKeyPairForExport,
				contentType: .text,
				defaultFilename: "Key.pem"
			) { (_) in }
			.confirmationDialog(
				"Delete \(self.selectedKeyPairs.count) \(self.selectedKeyPairs.count == 1 ? "Key" : "Keys")",
				isPresented: self.$doShowConfirmationDialog
			) {
